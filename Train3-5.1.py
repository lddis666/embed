...
class ASPathDataset(Dataset):
    ...
    def build_split_pairs(self, paths):
        """
        对每条路径随机预生成一些切割点。
        简化做法：每条路径生成若干 (A_idx_range, B_idx_range)。
        这里简单处理：每条路径生成若干随机切点。
        """
        pairs = []
        for pid, p in enumerate(paths):
            if len(p) < 2:
                continue
            # 生成若干切点
            num_cuts = min(5, len(p) - 1)
            cut_positions = random.sample(range(1, len(p)), num_cuts)
    ...
    def __len__(self):
        return len(self.split_pairs)

    def __getitem__(self, idx):
        """
        返回：
            input_ids: [CLS] A [SEP] B [SEP] + padding
        ...
        if random.random() < 0.5:
            ...
            # 从别的路径中随机取一个子路径作为 B
            while True:
                pid2, A2, B2 = random.choice(self.split_pairs)
                if pid2 != pid:
                    B = B2
                    break

        # 截断（给 A,B 留一定长度），简单按 max_seq_len 拆
        # 布局: [CLS] A [SEP] B [SEP] 最多 max_seq_len
        # 预留 3 个特殊 token
        max_len_for_AB = self.max_seq_len - 3
        half = max_len_for_AB // 2
        if len(A) > half:
            A = A[:half]
        if len(B) > max_len_for_AB - len(A):
            B = B[: (max_len_for_AB - len(A))]

        # 转换为 token idx
        def asnIdx_list_to_tokenIdx_list(asn_list):
            return [self.asnIdx2tokenIdx[a] for a in asn_list]

        A_tok = asnIdx_list_to_tokenIdx_list(A)
        B_tok = asnIdx_list_to_tokenIdx_list(B)

        tokens = [CLS_ID] + A_tok + [SEP_ID] + B_tok + [SEP_ID]
        seg_ids = [0] * (len(A_tok) + 2) + [1] * (len(B_tok) + 1)
        assert len(tokens) == len(seg_ids)
...
class ASBertModel(nn.Module):
    ...
    def forward(self,
        ...
        # NSP 头 (句子级)，用 [CLS] 位置
        cls_h = h[:, 0, :]                  # (B, d_model)
        nsp_logits = self.nsp_head(cls_h)   # (B, 2)
...
if __name__ == "__main__":
    ...
    dataset = ASPathDataset(paths_idx, asnIdx2tokenIdx, cfg.max_seq_len, cfg.mask_prob, F_prime, miss_ratio)
    ...
    for epoch in range(cfg.num_epochs):
        for batch in dataloader:
            ...
            nsp_label = batch["nsp_label"].to(cfg.device)
